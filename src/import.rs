...
/// Quiet import for TUI — returns Ok(summary) or Err(message), no stderr output.
/// Parsing always completes; sync errors are reported in the Ok message, not as failures.
/// If `selected_dirs` is provided, only imports sessions from those project directories.
pub fn run_import(dir: &Path, selected_dirs: Option<&std::collections::HashSet<String>>, do_sync: bool) -> Result<String, String> {
    let claude = claude_dir();
    if !claude.exists() {
        return Err("~/.claude not found".to_string());
    }

    let discovered = discover_sessions(&claude, selected_dirs);
    ...
    for chunk in all_session_json.chunks(50) {
